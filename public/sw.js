...
const DATA_CACHE_MAX_AGE_MS = 60 * 60 * 1000;
...
self.addEventListener('fetch', (event) => {
  ...
  if (url.pathname.startsWith('/api/')) {
    ...
    const shouldCacheApi = request.method === 'GET' && DATA_URLS.includes(apiPath);
        ...
        .then((response) => {
          ...
          if (response.status === 200 && request.method === 'GET') {
            caches.open(DATA_CACHE)
              .then((cache) => {
                // Set cache timestamp (used to enforce a short TTL on read-only cached API responses)
          ...
          return caches.open(DATA_CACHE)
            .then((cache) => {
              return cache.match(request)
                .then((cachedResponse) => {
                  if (cachedResponse) {
                    // Check if cache is still valid (short TTL)
                    const cacheTimestamp = cachedResponse.headers.get('sw-cache-timestamp');
                    const cacheAge = Date.now() - parseInt(cacheTimestamp || '0');
                    const maxAge = DATA_CACHE_MAX_AGE_MS;
