...
# This script deploys the backend application to the Tencent Lighthouse server for staging.
...
# - Exclude production env files but allow staging env files
...
echo "Copying ecosystem-staging.config.js to server..."
scp -i ${SSH_KEY_PATH} ecosystem-staging.config.js ${SERVER_USER}@${SERVER_IP}:${REMOTE_BASE_DIR}/ecosystem-staging.config.js
...
ssh -i ${SSH_KEY_PATH} ${SERVER_USER}@${SERVER_IP} << EOF
 echo "Navigating to project directory..."
  ...
  echo "Stopping existing staging backend process (if running)..."
  pm2 stop loom-backend-staging || true
  pm2 delete loom-backend-staging || true

  echo "Starting staging backend application with pm2 using ecosystem file..."
